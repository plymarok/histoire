...
const triData = [
  ...
  // ... continuer les 12 questions (même modèle que dans la liste validée) ...
];
  ...
  ["f1","f2","f3"].forEach((key,j)=>{
    ...
    f.addEventListener("click",()=>{
      ...
      if(f.classList.contains("open")){
        f.innerHTML=`<strong>F${j+1} :</strong> ${item[key].q}<br><em>Réponse :</em> ${item[key].a}<br><small>${item[key].src}</small>`;
      } else {
        f.innerHTML=`<strong>F${j+1} :</strong> ${item[key].q}`;
      }
    });
